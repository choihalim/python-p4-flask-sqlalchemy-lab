...
def animal_by_id(id):
    ...
    response_body = f'''
        ...
        <ul>Zookeeper: {Zookeeper.query.filter(Zookeeper.id == animal.zookeeper_id).first().name}</ul>
        <ul>Enclosure: {Enclosure.query.filter(Enclosure.id == animal.enclosure_id).first().environment}</ul>
    '''
...
def zookeeper_by_id(id):
    ...
    if not animals:
        response_body += f'<ul>No animals found for this zookeeper.</ul>'
    else:
...
def enclosure_by_id(id):
    ...
    if not animals:
        response_body += f'<ul>No animals found for this enclosure.</ul>'
    else:
        for animal in animals:
            response_body += f'<ul>Animal: {animal.name}</ul>'
